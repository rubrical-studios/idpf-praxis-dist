...
const MIGRATIONS = [
  {
    ...
    migrate: (projectDir, frameworkPath, config) => {
      ...
      // Create .claude/rules/ with new structure (v0.19.1+: singular domainSpecialist)
...
function runMigrations(projectDir, frameworkPath) {
  ...
  // Clean up orphaned files after all migrations (v0.19.1+: singular domainSpecialist)
  const hasGitHubWorkflow = fs.existsSync(path.join(projectDir, '.claude', 'hooks', 'workflow-trigger.js'));
  const cleanupConfig = {
