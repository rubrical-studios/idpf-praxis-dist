...
describe('Manifest Validation', () => {
  ...
  describe('@framework-script tag (prevent #1019 regression)', () => {
    // All framework JS files must have @framework-script 0.33.1 tag
    const jsDirectories = [
    ...
    test.each(jsDirectories)(
      ...
      ({ path: dirPath }) => {
        ...
        if (missingTag.length > 0) {
          throw new Error(
            `Files missing @framework-script 0.33.1 tag:\n  ${missingTag.join('\n  ')}\n` +
            `Add: @framework-script 0.33.1 as first line in JSDoc block.`
          );
        }
      }
    );
