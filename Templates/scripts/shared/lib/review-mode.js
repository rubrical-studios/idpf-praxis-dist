 ...
 */
function getReviewMode(projectDir = process.cwd()) {
 ...
 */
function shouldEvaluate(criterionId, projectDir = process.cwd()) {
  const criteria = loadCriteriaMapping();
  const currentMode = getReviewMode(projectDir);
 ...
 */
function getApplicableCriteria(projectDir = process.cwd()) {
  const criteria = loadCriteriaMapping();
  const currentMode = getReviewMode(projectDir);
 ...
 */
function getObjectiveCriteria(projectDir = process.cwd()) {
  return getApplicableCriteria(projectDir).filter(c => c.type === 'objective');
}
 ...
 */
function getSubjectiveCriteria(projectDir = process.cwd()) {
  return getApplicableCriteria(projectDir).filter(c => c.type === 'subjective');
}
 ...
 */
function getFilteredCriteria(projectDir = process.cwd()) {
  const criteria = loadCriteriaMapping();
  const currentMode = getReviewMode(projectDir);
